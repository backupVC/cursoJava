...
INSERT INTO usuario(nombre, dni) VALUES ('juan gabriel', '11111111');
INSERT INTO usuario(nombre, dni) VALUES ('pedro ramónl', '22222222');
INSERT INTO usuario(nombre, dni) VALUES ('luis felipe', '33333333');
INSERT INTO usuario(nombre, dni) VALUES ('remedios amayal', '44444444');
INSERT INTO usuario(nombre, dni) VALUES ('patricia conde', '55555555');
INSERT INTO usuario(nombre, dni) VALUES ('enrique luis', '66666666');
...
CREATE VIEW librosCategorias AS SELECT libro.nombre, libro.autor, libro.editorial, categoria.nombre FROM libro, categoria WHERE libro.idCategoria=categoria.id;
--libros prestados por categorías
CREATE VIEW librosCategoriasPrestados AS SELECT DISTINCT libro.nombre, libro.autor, libro.editorial, categoria.nombre FROM libro, categoria, prestamo WHERE libro.idCategoria=categoria.id AND libro.id = prestamo.idLibro; 
--libros no prestados por categorías
CREATE VIEW librosNoPrestadosCategorias AS SELECT libro.nombre, autor, editorial, categoria.nombre FROM libro, categoria WHERE libro.id NOT IN (SELECT idLibro FROM prestamo) AND libro.idCategoria=categoria.id;
--libros prestados 
...
CREATE VIEW librosPrestadosUsuario AS SELECT libro.nombre, autor, editorial, usuario.nombre FROM libro, usuario, prestamo WHERE libro.id=prestamo.idLibro AND usuario.id = prestamo.idUsuario;
...
ON usuario
BEGIN
INSERT INTO historial_usuario (nombre, dni, fecha_baja)
VALUES (old.nombre, old.dni, datetime('now'));
END;
